...
/* ---------- 2b. MODAL AKTIVITAS PBL (Mean, Median, Modus) ---------- */
(() => {
  const openBtn = document.getElementById('pblOpenBtn');
  ...
  const tabs = Array.from(document.querySelectorAll('.pbl-tab'));
  const panels = Array.from(document.querySelectorAll('.pbl-tab-panel'));
  const dots = Array.from(document.querySelectorAll('.pbl-dot'));
  ...
  if (!openBtn || !overlay || !tabs.length) return;
  ...
  function activateTab(index) {
    ...
    modal.querySelector('.pbl-modal-body').scrollTo({ top: 0, behavior: 'instant' in window ? 'instant' : 'auto' });
  }
  ...
  if (prevBtn) prevBtn.addEventListener('click', () => activateTab(currentIndex() - 1));
  if (nextBtn) {
